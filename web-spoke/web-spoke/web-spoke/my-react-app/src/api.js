...
async function http(method, path, { json, formData } = {}) {
  ...
  try {
    ...
    if (isNetworkError(e)) {
      const configuredBase = getRuntimeApiBase() || "same-origin /api";
      throw new Error(`Cannot reach hub API at ${configuredBase}.`);
    }
    throw e;
  }
...
export const api = {
  ...
  photoViewUrl(photoId, token) {
    ...
    return toApiUrl(`/api/photos/${photoId}/view${q}`);
  },

  photoDownloadUrl(photoId, token) {
    const q = token ? `?token=${encodeURIComponent(token)}` : "";
    return toApiUrl(`/api/photos/${photoId}/download${q}`);
  },
